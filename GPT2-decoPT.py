...
# Run testing tracking through DDP layer cleanly
# loss, logits = model(
#     input_ids=input_ids,
#     labels=input_ids,
# )
...
# class RotaryEmbedding(nn.Module):
#     """
#     Rotary positional embedding for decoder-only self-attention.

#     Applies RoPE to query/key tensors shaped: (batch, n_heads, seq_len, head_dim)
...
#         # Shape for broadcasting over batch and heads:
...
# class GroupedQueryAttention(nn.Module):
#     """
#     GPT-style Grouped Query Attention with optional FlashAttention.
#     Input: x: (batch, seq_len, n_embd)
#     Optional: attention_mask: (batch, seq_len), where 1 = valid token, 0 = padding
#     Output: y: (batch, seq_len, n_embd)
#     """
...
#         # q: (B, T, n_query_heads, head_dim)
#         # k/v: (B, T, n_kv_heads, head_dim)
#         q = q.view(B, T, self.n_query_heads, self.head_dim)
#         k = k.view(B, T, self.n_kv_heads, self.head_dim)
#         v = v.view(B, T, self.n_kv_heads, self.head_dim)

#         # If using RoPE, apply it here before attention:
#         # q, k = self.rope(q, k)
#         #
#         # Note: if your RoPE expects (B, H, T, D), transpose before/after.

#         # if self._can_use_flash_attention(q, k, v):
#         #     y = self._flash_attention(q, k, v, attention_mask)
#         # else:
#         y = self._torch_attention(q, k, v)
...
#     def _torch_attention(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
#         B, T, _, _ = q.shape

#         # PyTorch attention expects:
#         # q/k/v: (B, heads, T, head_dim)
#         q = q.transpose(1, 2)
#         k = k.transpose(1, 2)
#         v = v.transpose(1, 2)

#         # PyTorch SDPA generally expects the same number of q/k/v heads,
#         # so repeat K/V for the fallback path.
#         k = repeat_kv(k, self.n_rep)
#         v = repeat_kv(v, self.n_rep)
...
#         # Return to: (B, T, n_query_heads, head_dim)
...
#     Output:
#         x: (batch, n_kv_heads * n_rep, seq_len, head_dim)
#     """
...
# class PredictionHead(nn.Module):
#     """
#     Language modeling head for a GPT-style decoder.
...
# class GPT2DecoderModel(nn.Module):
#     """
#     GPT-style decoder-only language model for next-token prediction.
#     Assumes you already have:
#     - RMSNorm
#     - TransformerDecoderLayer
#     - PredictionHead
#     - RotaryEmbedding or RoPE cache support inside attention
#     """
...
#         # No learned positional embedding if using RoPE.
#         # RoPE is usually cached/applied inside attention.
#         self.layers = nn.ModuleList(
#             [
#                 TransformerDecoderLayer(
#                     hidden_size=hidden_size,
...
#     def tie_weights(self):
#         """
#         Tie token embedding weights with the LM prediction head.
#         This is standard in GPT-style models.
#         """
#         self.prediction_head.lm_head.weight = self.token_embeddings.weight
...
#         attention_mask: optional, (batch, seq_len), where 1 = valid token, 0 = padding
...
# # model = Encoder(vocab_size=32000, hidden_size=512, num_layers=16, num_heads=8, num_kv_heads=2, max_seq_len=128)

# input_ids = torch.randint(0, 32000, (8, 128)) #50257

# model = GPT2DecoderModel(
#     vocab_size=32000, #50257
#     hidden_size=512, #768
#     num_layers=16, #12
#     num_heads=8,#12
#     num_kv_heads=4, #4
...
# print(loss)
# print(logits.shape)  # (8, 1024, 32000)  #50257
...
# save_path = "hindi_chunksGP.npy"
# chunks = np.save(save_path)

# train_dataset = GPT2Dataset(chunks) #your_list_of_chunks, GPT2Dataset
